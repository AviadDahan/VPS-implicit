...
class IMLP(nn.Module):
    def __init__(
        ...
        for i in range(num_layers):
            if i == 0:
                input_dims = encoding_dimensions + positional_dim
            elif i in skip_layers:
                input_dims = hidden_dim + encoding_dimensions + positional_dim
            else:
        ...
        self.embedding = nn.Embedding(num_of_ex, positional_dim)
    ...
    def forward(self, x, ex):
        if self.use_positional:
            ...
            z_ex = self.embedding(ex)
            x = torch.cat((pos, z_ex), 1)
