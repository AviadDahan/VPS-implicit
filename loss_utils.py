...
def get_optical_flow_alpha_loss(model_alpha, criterion,
    ...
    # alpha_foreground_forward_should_match = torch.argmax(alpha_foreground_forward_should_match, dim=1).detach()
    # loss_flow_alpha_next = criterion(alpha[relevant_batch_indices_forward], alpha_foreground_forward_should_match)
    ...
    # alpha_foreground_backward_should_match = torch.argmax(alpha_foreground_backward_should_match, dim=1).detach()
    # loss_flow_alpha_prev = criterion(alpha[relevant_batch_indices_backward], alpha_foreground_backward_should_match)
    return loss_flow_alpha_next, loss_flow_alpha_prev
...
# Compute alpha optical flow loss (Eq. 12 in the paper)
def get_optical_flow_alpha_loss_binry(model_alpha, ex,
                                      jif_foreground, alpha, optical_flows_reverse, optical_flows_reverse_mask, resx,
                                      number_of_frames, optical_flows, optical_flows_mask, device):
    # Forward flow
    xyt_foreground_forward_should_match, relevant_batch_indices_forward = get_corresponding_flow_matches(
