...
def load_input_data(resy, resx, maximum_number_of_frames, data_folder, file, args):
    out_flow_dir = data_folder / 'flow' / file
    if args['dataset'] == 'davis2017':
        data_folder = data_folder / 'JPEGImages' / '480p' / file
    elif args['dataset'] == 'LiTS':
        data_folder = data_folder / 'jpg' / file
    else:
        data_folder = data_folder / 'Frame' / file

    input_files = natsorted(list(data_folder.glob('*.jpg')) + list(data_folder.glob('*.png')))
    ...
    return optical_flows_mask, video_frames, optical_flows_reverse_mask, video_frames_dx, video_frames_dy, \
           optical_flows_reverse, optical_flows
